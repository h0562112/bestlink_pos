		...
		?>
			...
			if(isset($secview['leftimg']['imgnum'])){
				for($i=1;$i<=$secview['leftimg']['imgnum'];$i++){
					echo "<img style='width:100%;height:100%;object-fit:contain;display: block;' src='./img/imglist/".$i.".png?".date('YmdHis')."'>";
				}
			}
			else{
				...
				foreach($filelist as $v){
					if(!in_array($v,array(".","..","Thumbs.db"))){
						if (is_dir('./img/imglist' . DIRECTORY_SEPARATOR . $v)){
							...
							echo "<img style='width:100%;height:100%;object-fit:contain;display: block;' src='./img/imglist/".$v."?".date('YmdHis')."'>";
						}
					}
					else{
					}
