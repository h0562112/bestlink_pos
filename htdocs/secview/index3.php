...
<body>
	...
	<div id='left' style='width:70%;height:100%;float:left;position:relative;'>
		<?php
		...
		?>
			<div id='prev'>
				<img src='./img/left.png' style='width:80px;'>
			</div>
			<div class='cycle-slideshow dyphoto' data-cycle-fx='scrollHorz' data-cycle-timeout='<?php echo floatval($secview['leftimg']['imgspeed'])*1000; ?>' data-cycle-next="#next" data-cycle-prev="#prev" style='background-image:url("");width:100%;height:100%;padding:0;margin:0;float:left;'>
				<?php
					...
					foreach($filelist as $v){
						if(!in_array($v,array(".","..","Thumbs.db"))){
							if (is_dir('./img/imglist' . DIRECTORY_SEPARATOR . $v)){
								...
								echo "<img style='width:100%;height:100%;object-fit:contain;' src='./img/imglist/".$v."?".date('YmdHis')."'>";
							}
						}
						else{
						}
			...
			<div id='next'>
				<img src='./img/right.png' style='width:80px;'>
			</div>
		<?php
		}
		?>
