...
<head>
	...
	<script>
		...
		$(document).ready(function(){
			...
			$('.viewtemp #searchman #sendmancode').click(function(){
				if($('.viewtemp #searchman input[name="manname"]').val()!=''){//2021/10/29 查詢外送員送的單
					$.ajax({
						url:'../demopos/lib/js/getsalelist.ajax.php',
						method:'post',
						data:{'temp':'temp','machinetype':$('#setup input[name="machinetype"]').val(),'mancode':$('.viewtemp #searchman input[name="manname"]').val()},
						dataType:'html',
						success:function(d){
							$('.viewtemp .reorder').prop('disabled',true);
							$('.viewtemp #salecontent').html(d);
			...
			$('.viewtemp .exit').click(function(){
				...
				$('.viewtemp #searchman input[name="manname"]').val('');
				$('.setwin').css({'display':'block'});
				$('.modal').css({'display':'block'});
			});
...
<body>
	...
	<div class='viewtemp' style='width:calc(100% - 12px);height:calc(100% - 12px);border:1px solid rgb(74,74,74,0.5);position:fixed;top:0;left:0;z-index:2020;padding:5px;background-color:#ffffff;display:none;overflow:hidden;font-size:16px;'>
		...
		<div id='searchman' style='width:calc(100% - 14px);height:calc(66px);padding:0 5px;margin:0 4px 0 0;overflow:auto;'>
			<input type='tel' name="manname" placeholder="員工編號" style="width:calc((100% - 4px) / 2);height:calc(100% - 21px);border:0px;border-bottom:1px solid #898989;border-radius:0;padding:0;margin:10px 2px 10px 0;float:left;">
			<button id="sendmancode" style="width:calc((100% - 4px) / 2);height:calc(100% - 21px);float:left;margin:10px 0 10px 2px;border:1px solid #898989;border-radius:5px;">查詢</button>
		</div>
		<div id='salecontent' style='width:calc(100% - 14px);height:calc((100% - 108px - 66px) / 2 - 6px);padding:0 5px;margin:2px 0 4px 0;border:1px solid #898989;border-radius:5px;overflow:auto;box-shadow: rgba(0, 0, 0, 0.2) 2px 2px;'></div>
		<div id='listcontent' style='width:calc(100% - 14px);height:calc((100% - 108px - 66px) / 2 - 6px);padding:0 5px;margin:2px 0 4px 0;border:1px solid #898989;border-radius:5px;overflow:auto;box-shadow: rgba(0, 0, 0, 0.2) 2px 2px;'></div>
		<div style='width:calc(100% - 10px);height:50px;padding:0 5px;'>
