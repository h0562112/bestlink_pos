...
if(file_exists('../../../database/sale/SALES_'.substr($bizdate,0,6).'.db')){
	...
	if(isset($_POST['sale'])&&$_POST['sale']=='sale'){
		...
		// $sql = 'SELECT 
        //     tempCST011.TERMINALNUMBER, tempCST011.BIZDATE, tempCST011.CONSECNUMBER, tempCST011.CLKCODE, 
        //     tempCST011.CLKNAME, tempCST011.CUSTGPCODE, tempCST011.CUSTGPNAME, tempCST011.CUSTCODE, tempCST011.CUSTNAME,
        //     tempCST011.INVOICENUMBER, tempCST011.SALESTTLAMT, tempCST011.NBCHKNUMBER, tempCST011.ZCOUNTER, 
        //     tempCST011.REMARKS, tempCST011.CREATEDATETIME, tempCST011.UPDATEDATETIME, tempCST011.TABLENUMBER
        // FROM 
		// 	tempCST011 

        // WHERE 
        //     tempCST011.BIZDATE = "'.$bizdate.'"';
		...
		if(isset($_POST['mancode'])&&$_POST['mancode']!=''){//2021/10/29 orderpos暫結查詢外送員的單
			...
			if($inputDate == "") {
				$sql='SELECT 
				(SELECT COUNT(*) FROM tempCST011 WHERE NBCHKNUMBER IS NULL AND tempCST011.BIZDATE =  "'.$bizdate.'") AS ttcount,
				(SELECT SUM(SALESTTLAMT + TAX1 + TAX9) FROM tempCST011 WHERE NBCHKNUMBER IS NULL AND tempCST011.BIZDATE =  "'.$bizdate.'") AS ttmoney,
				TERMINALNUMBER,CONSECNUMBER,BIZDATE,CLKCODE,CLKNAME,CUSTGPCODE,CUSTGPNAME,CUSTCODE,CUSTNAME,INVOICENUMBER,
				(SALESTTLAMT + TAX1 + TAX9) AS SALESTTLAMT,NBCHKNUMBER,ZCOUNTER,REMARKS,CREATEDATETIME,UPDATEDATETIME,TABLENUMBER 
				FROM tempCST011 
				WHERE NBCHKNUMBER IS NULL AND tempCST011.BIZDATE =  "'.$bizdate.'"
				ORDER BY substr(CREATEDATETIME||"000",1,17) DESC';
			}else{
				$sql='SELECT 
				(SELECT COUNT(*) FROM tempCST011 WHERE NBCHKNUMBER IS NULL AND tempCST011.BIZDATE =  "'.$inputDate.'") AS ttcount,
				(SELECT SUM(SALESTTLAMT + TAX1 + TAX9) FROM tempCST011 WHERE NBCHKNUMBER IS NULL AND tempCST011.BIZDATE =  "'.$inputDate.'") AS ttmoney,
				TERMINALNUMBER,CONSECNUMBER,BIZDATE,CLKCODE,CLKNAME,CUSTGPCODE,CUSTGPNAME,CUSTCODE,CUSTNAME,INVOICENUMBER,
				(SALESTTLAMT + TAX1 + TAX9) AS SALESTTLAMT,NBCHKNUMBER,ZCOUNTER,REMARKS,CREATEDATETIME,UPDATEDATETIME,TABLENUMBER 
				FROM tempCST011 
				WHERE NBCHKNUMBER IS NULL AND tempCST011.BIZDATE =  "'.$inputDate.'"
				ORDER BY substr(CREATEDATETIME||"000",1,17) DESC';
			}
			// 舊的未結清單查詢語句該語句為一次查詢tempCST011資料表內全部項目
			// $sql='SELECT (SELECT COUNT(*) FROM tempCST011 WHERE NBCHKNUMBER IS NULL) AS ttcount,(SELECT SUM(SALESTTLAMT + TAX1 + TAX9) FROM tempCST011 WHERE NBCHKNUMBER IS NULL) AS ttmoney,TERMINALNUMBER,CONSECNUMBER,BIZDATE,CLKCODE,CLKNAME,CUSTGPCODE,CUSTGPNAME,CUSTCODE,CUSTNAME,INVOICENUMBER,(SALESTTLAMT + TAX1 + TAX9) AS SALESTTLAMT,NBCHKNUMBER,ZCOUNTER,REMARKS,CREATEDATETIME,UPDATEDATETIME,TABLENUMBER FROM tempCST011 WHERE NBCHKNUMBER IS NULL ORDER BY substr(CREATEDATETIME||"000",1,17) DESC';
		}
	}
	else{
