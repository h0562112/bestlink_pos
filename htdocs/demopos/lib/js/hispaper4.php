...
if(file_exists($filedb)){
	...
	if(!$conn){
		...
		if(isset($res[0]['name'])){
			$sql="SELECT BIZDATE,SUM(TAX6+TAX7+TAX8) AS AMT,SUBSTR(CREATEDATETIME,9,2) AS SALETIME FROM (SELECT BIZDATE,TAX6,TAX7,TAX8,SALESTTLAMT,CASE WHEN UPDATEDATETIME='0' THEN CREATEDATETIME ELSE UPDATEDATETIME END AS CREATEDATETIME FROM CST011 WHERE BIZDATE";
			if(isset($_POST['papbizdateS'])&&isset($_POST['papbizdateE'])&&$_POST['papbizdateS']!=''&&$_POST['papbizdateE']!=''){
	...
	if(!isset($_POST['type'])||$_POST['type']!='view'){
		...
		if($paper!='-1'){
			$document->setValue('title',$paper['historypaper']['histitle4']);
		}
		else{
			$document->setValue('title','時段來客數彙總');
		}
		...
		if($paper!='-1'){
			$table .= $paper['historypaper']['his4title2'];
		}
		else{
			$table .= "來客數";
		}
				...
				echo '<table style="border-collapse:collapse;">';
					echo '<tr>
						...
						<tr>
							...
							if($paper!='-1'){
								echo $paper['historypaper']['histitle4'];
							}
							else{
								echo '時段來客數彙總';
							}
		...
		echo '<tr>
			...
			echo '</td>
				<td>';
				if($paper!='-1'){
					echo $paper['historypaper']['his4title2'];
				}
				else{
					echo "來客數";
				}
