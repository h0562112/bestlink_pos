...
ksort($data);//2022/9/26 針對array key排序
...
fwrite($f,date('Y/m/d H:i:s').' --- url: '.$_POST['url'].'/'.$_POST['SystemName'].'/Payment'.PHP_EOL);
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $_POST['url'].'/'.$_POST['SystemName'].'/Payment');
...
$Result = curl_exec($ch);
if(curl_errno($ch) !== 0) {
	print_r('cURL error when connecting to ' . $_POST['url'].'/'.$_POST['SystemName'].'/Payment' . ' : ' . curl_error($ch));
}
curl_close($ch);
