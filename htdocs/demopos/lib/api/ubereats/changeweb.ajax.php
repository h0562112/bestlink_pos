...
if(isset($_POST['data'])){
	foreach($_POST['data'] as $d){
		...
		$sql1='UPDATE cst011 SET ORDERTYPE="2",listbizdate="'.$d[0].'" WHERE CLKNAME="UberEats" AND TERMINALNUMBER="'.$d[4].'" AND ORDERTYPE="-1" AND CONSECNUMBER="'.$d[3].'" AND BIZDATE="'.$d[5].'"';
		sqlnoresponse($conn,$sql1,'mysql');

		//2022/1/26 加入判斷CLKNAME=UberEats
		$sql2='UPDATE cst012 SET ORDERTYPE="2" WHERE CLKNAME="UberEats" AND TERMINALNUMBER="'.$d[4].'" AND ORDERTYPE="-1" AND CONSECNUMBER="'.$d[3].'" AND BIZDATE="'.$d[5].'"';
