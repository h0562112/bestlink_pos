<?php
if($_POST['searchtype']=='cell'){
	$data=array(
		"uid" => $_POST['uid'],
		"token" => $_POST['token'],
		"cell" => $_POST['searchdata'],
		"field" => "point,vip"
	);
}
else{
...
fwrite($f,date('Y/m/d H:i:s').' --- url: '.$_POST['url'].'/getProfile'.PHP_EOL);
fwrite($f,'                    --- data: '.print_r($data,true).PHP_EOL);

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $_POST['url'].'/getProfile');
...
$Result = curl_exec($ch);
if(curl_errno($ch) !== 0) {
	print_r('cURL error when connecting to ' . $_POST['url'].'/getProfile' . ' : ' . curl_error($ch));
}
curl_close($ch);
