...
fwrite($f,date('Y/m/d H:i:s').' --- url: '.$_POST['url'].'/giveVip'.PHP_EOL);
fwrite($f,'                    --- data: '.print_r($data,true).PHP_EOL);

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $_POST['url'].'/giveVip');
...
$Result = curl_exec($ch);
if(curl_errno($ch) !== 0) {
	print_r('cURL error when connecting to ' . $_POST['url'].'/giveVip' . ' : ' . curl_error($ch));
}
curl_close($ch);
