				...
				if(istrue==0){//由於統編格示錯誤，不做處理
					...
					/*if($('.result #paywindow .paywaybox .paycash').length>0&&$('.order#order .initsetting #creditcode').val()=='1'){//使用信用卡付款
						...
						/*$.ajax({
							...
							method:'post',
							async:false,
							data:{'company':$('.order#order .companydata #company').val(),'machinetype':$('.order#order #tabs4 form[data-id="listform"] input[name="machinetype"]').val(),'consecnumber':consecnumber},
							dataType:'json',
							success:function(d){
								//console.log(d);
								if(d[0].length==0){
