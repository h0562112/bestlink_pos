...
$sortrear=quicksort($rearname,'seq');
...
for($i=0;$i<sizeof($sortrear);$i++){
	if(isset($sortrear[$i]['state'])&&$sortrear[$i]['state']=='0'){
		echo "<tr class='typerow'>";
		echo "<td style='width:62px;'><img id='chimg' src='./img/noch.png'><input type='checkbox' style='display:none;'><input type='hidden' name='number' value='".$sortrear[$i]['typeno']."'></td>";
		echo "<td>".$sortrear[$i]['name'.$initsetting['init']['firlan']]."</td>";
		echo "<td>".$sortrear[$i]['name'.$initsetting['init']['seclan']]."</td>";
		echo "</tr>";
	}
	else{
	}
...
/*for($i=0;$i<sizeof($rearname);$i++){
	if($rearname[$i]['state']=='1'){
		...
		echo "<td>".$rearname[$i]['name'.$initsetting['init']['firlan']]."</td>";
		echo "<td>".$rearname[$i]['name'.$initsetting['init']['seclan']]."</td>";
		echo "</tr>";
	}
	else{
	}
