...
if($_SESSION['DB']==''){
	$machinedata=parse_ini_file('../../../ourpos/'.$_SESSION['company'].'/'.$_POST['dbname'].'/machinedata.ini',true);
				...
				if(!isset($emptyinv)||sizeof($emptyinv)==0){
					...
					//echo '<div>';
						if((intval(substr($startinv[0]['banno'],2))+intval($machinedata['inv']['total'])*50)>intval(substr($endinv[0]['banno'],2))){
							fwrite($f,'共有 '.(sizeof($emptyinv)+((intval(substr($startinv[0]['banno'],2))+intval($machinedata['inv']['total'])*50-1)-intval(substr($endinv[0]['banno'],2)))).'張 空白發票'.PHP_EOL);
						...
						if(intval($start)<intval(substr($emptyinv[sizeof($emptyinv)-1]['banno'],2))&&intval(substr($emptyinv[sizeof($emptyinv)-1]['banno'],2))<=intval((intval(substr($startinv[0]['banno'],2))+intval($machinedata['inv']['total'])*50-1))){
							fwrite($f,$en.str_pad((intval(substr($startinv[0]['banno'],2))+intval($machinedata['inv']['total'])*50-1),8,"0",STR_PAD_LEFT).PHP_EOL);
