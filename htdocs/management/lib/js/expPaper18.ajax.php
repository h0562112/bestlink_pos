...
if(isset($_POST['startdate'])){
	...
	if(isset($machinemap)&&sizeof($machinemap)==1){
		fwrite($f,'日期,開立張數,開立金額(不含作廢),起號,迄號,作廢張數,作廢金額,作廢發票'.PHP_EOL);
	}
	else{
		fwrite($f,'日期,機號,開立張數,開立金額(不含作廢),起號,迄號,作廢張數,作廢金額,作廢發票'.PHP_EOL);
	}
							...
							if($i!=0){
								...
								if($machineindex==1){
									switch(date('N',strtotime($date))){
										case 1:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(一)');
											break;
										case 2:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(二)');
											break;
										case 3:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(三)');
											break;
										case 4:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(四)');
											break;
										case 5:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(五)');
											break;
										case 6:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(六)');
											break;
										case 7:
											fwrite($f,substr($date,2,2).'/'.substr($date,4,2).'/'.substr($date,6,2).'(日)');
											break;
										default:
											break;
									}
