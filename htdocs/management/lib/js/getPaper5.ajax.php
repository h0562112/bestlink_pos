...
if(isset($_POST['startdate'])){
	...
	for($m=0;$m<=$totalMon;$m++){
		if(empty($_SESSION['DB'])||$_SESSION['DB']==''){
		...
		if(!$conn){
			...
			if(isset($res[0]['name'])){
				$sql="SELECT BIZDATE,SUM(SALESTTLAMT) AS AMT,SUBSTR(CREATEDATETIME,0,11) AS SALETIME FROM (SELECT BIZDATE,SALESTTLAMT,CASE WHEN UPDATEDATETIME='0' THEN CREATEDATETIME ELSE UPDATEDATETIME END AS CREATEDATETIME FROM CST011 WHERE BIZDATE BETWEEN '".$start."' AND '".$end."' AND NBCHKNUMBER IS NULL) GROUP BY SUBSTR(CREATEDATETIME,0,11) ORDER BY SALETIME";
		...
		for($t=-1;$t<=$maxhour;$t++){
			if($t==-1){
				...
				for($i=strtotime(date($start));$i<=strtotime(date($end)),$i<=strtotime(date("Y-m-d"));$i=strtotime(date("Y-m-d",$i).' +1 days')){
					if(strtotime(date($end))>=strtotime(date("Y-m-d"))&&$i>strtotime(date("Y-m-d"))){
						break;
					}
					else if(strtotime(date($end))<strtotime(date("Y-m-d"))&&$i>strtotime(date($end))){
						break;
					}
					else{
					}
					...
					echo "</td>";
					//echo "<td style='padding:5px;'>".substr(date("Ymd",$i),2,6)."</td>";
				}
				echo '</tr>';
				echo '</thead>';
			}
					...
					if(isset($data[$t])){
						...
						for($i=strtotime(date($start));$i<=strtotime(date($end)),$i<=strtotime(date("Y-m-d"));$i=strtotime(date("Y-m-d",$i).' +1 days')){
							...
							if(isset($data[$t][date("Ymd",$i)])){
								echo $unit['init']['frontunit'].number_format($data[$t][date("Ymd",$i)]).$unit['init']['unit'];
							}
							else{
								echo $unit['init']['frontunit'].'0'.$unit['init']['unit'];
							}
							echo "</td>";
						}
						echo '</tr>';
