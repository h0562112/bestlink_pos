...
if(isset($_POST['startdate1'])){
	...
	if($complete>=($totalMon+1)){
		...
		if(sizeof($list)==0){
			...
			//$list=array("1"=>array("qty"=>"20"),"2"=>array("qty"=>"15"),"3"=>array("qty"=>"2"),"4"=>array("qty"=>"60"),"5"=>array("qty"=>"65"),"6"=>array("qty"=>"1"),"7"=>array("qty"=>"21"));
...
for($i=0;$i<sizeof($sort);$i++){
	if($i>0&&$sort[$i]['qty']==$sort[$i-1]['qty']){
		$sort[$i]['rank']=$befrank;
	}
	else{
		$sort[$i]['rank']=$i+1;
...
for($i=0;$i<sizeof($sort2);$i++){
	if($i>0&&$sort2[$i]['qty']==$sort2[$i-1]['qty']){
...
for($i=0;$i<sizeof($sort);$i++){
	...
	echo '<td style="text-align:right;">'.number_format($sort[$i]['qty']).'</td>';
	if(in_array($sort[$i]['no'],$temp2)){
		echo '<td style="text-align:right;">'.number_format($sort2[array_search($sort[$i]['no'],$temp2)]['qty']).'</td>';
	}
	else{
		echo '<td style="text-align:right;"><em>0</em></td>';
	}
	echo '</tr>';
}
	echo '</tbody>';
