...
if(isset($_POST['startdate'])){
	...
	if(!isset($Cconn)||!$Cconn){
		...
		if(!in_array('intella',array_column($allcolumn,'name'))){
			...
			$sql='SELECT bizdate,consecnumber,SUM(salesttlamt + tax1 + tax9) AS amt,SUM(tax2) AS tax2,SUM(tax3) AS tax3,SUM(tax4) AS tax4,SUM(intella) as intella FROM list WHERE bizdate BETWEEN "'.$start.'" AND "'.$end.'" AND state=1 GROUP BY bizdate,consecnumber';
	...
	if($complete>=($totalMon+1)){
		...
		if(sizeof($list)==0){
			...
			echo '<table class="table" style="margin-top:10px;"><tr><td colspan="11" style="padding:5px;"></td></tr><tr><td style="text-align:center;">營業日</td><td>帳單號</td><td style="text-align:right;">總金額</td><td style="text-align:right;">現金</td><td style="text-align:right;">信用卡</td><td style="text-align:right;">其他</td><td colspan="3" style="text-align:center;">英特拉</td><td style="text-align:center;">結帳時間</td><td>作廢/註銷時間</td><td>備註</td></tr><tr><td colspan="7" style="text-align:right;"></td><td style="text-align:center;">合計金額</td><td style="text-align:right;font-weight:bold;">'.number_format($summoney).'</td><td colspan="3"></td></tr><tr><td colspan="6"></td><td style="text-align:center;">交易號</td><td style="text-align:center;">付款方式</td><td style="text-align:right;">付款金額</td><td colspan="3"></td></tr>';
			echo $list;
			echo '</table>';
			/*foreach($list as $l){
				echo $l;
